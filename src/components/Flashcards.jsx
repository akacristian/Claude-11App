...
export default function Flashcards() {
  ...
  function applyFilter(f) {
    setFilter(f)
    const list =
      f === 'all' ? DISHES.map((d) => d.id) : DISHES.filter((d) => d.sec === f).map((d) => d.id)
    setOrder(list)
    setIdx(0)
    setFlipped(false)
  }
  ...
  return (
    <div>
        ...
        >
          ...
          <div className="card-face absolute inset-0 rounded-2xl bg-white shadow-lg border border-slate-100 p-6 flex flex-col">
            <div className="flex items-center gap-2">
              ...
              {g.known[dish.id] && (
                <span className="text-xs font-bold text-emerald-600 ml-auto">✓ Mastered</span>
              )}
            </div>
      ...
      <div className="flex items-center gap-2 mt-4">
        <button
          onClick={() => go(-1)}
          className="flex-1 py-3 rounded-xl bg-white border border-slate-200 font-semibold active:scale-95"
        >
          ‹ Prev
        </button>
        <button
          onClick={master}
          className="flex-1 py-3 rounded-xl bg-emerald-500 text-white font-semibold active:scale-95"
        >
          ✓ Mastered
        </button>
        <button
