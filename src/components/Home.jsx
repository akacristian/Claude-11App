...
function StatCard({ icon, label, value, onClick }) {
  return (
    <button
      onClick={onClick}
      className="text-left rounded-xl bg-white p-3 shadow-sm border border-slate-100 active:scale-95 transition"
    >
...
function ModeCard({ onClick, icon, title, desc, color }) {
  return (
    <button
      onClick={onClick}
      className={`w-full text-left rounded-2xl bg-white p-4 shadow-sm border border-slate-100 flex gap-4 items-start active:scale-[.99] transition ${MODE_HOVER[color] || ''}`}
    >
