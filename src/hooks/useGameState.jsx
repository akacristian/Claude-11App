...
export function GameProvider({ children }) {
  ...
  const showFeedback = useCallback((ok) => {
    setData((d) => { beep(ok ? 'good' : 'bad', d.muted); return d })
    setFlash(ok ? 'good' : 'bad')
    clearTimeout(flashTimer.current)
    flashTimer.current = setTimeout(() => setFlash(null), 480)
  }, [])

  const blip = useCallback((type) => setData((d) => { beep(type, d.muted); return d }), [])

  const addXP = useCallback((n) => setData((d) => ({ ...d, xp: Math.max(0, d.xp + n) })), [])

  const bumpStreak = useCallback((ok) => setData((d) => {
    const streak = ok ? d.streak + 1 : 0
    return { ...d, streak, bestStreak: Math.max(d.bestStreak, streak) }
  }), [])

  const markKnown = useCallback((id) => setData((d) => {
    if (d.known[id]) return d
    return { ...d, known: { ...d.known, [id]: true }, xp: d.xp + 5 }
  }), [])

  const recordQuiz = useCallback((ok) => setData((d) => ({
    ...d,
    xp: ok ? d.xp + 10 : d.xp,
    streak: ok ? d.streak + 1 : 0,
    bestStreak: ok ? Math.max(d.bestStreak, d.streak + 1) : d.bestStreak,
    stats: { ...d.stats, quizTotal: d.stats.quizTotal + 1, quizCorrect: d.stats.quizCorrect + (ok ? 1 : 0) },
  })), [])

  const recordGuard = useCallback((ok) => setData((d) => ({
    ...d,
    xp: ok ? d.xp + 12 : d.xp,
    streak: ok ? d.streak + 1 : 0,
    bestStreak: ok ? Math.max(d.bestStreak, d.streak + 1) : d.bestStreak,
    stats: { ...d.stats, guardTotal: d.stats.guardTotal + 1, guardCorrect: d.stats.guardCorrect + (ok ? 1 : 0) },
  })), [])

  const bumpFlashSeen = useCallback(() => setData((d) => ({ ...d, stats: { ...d.stats, flashSeen: d.stats.flashSeen + 1 } })), [])
  ...
  const value = {
    ...data, flash,
    addXP, bumpStreak, markKnown, recordQuiz, recordGuard, bumpFlashSeen,
    toggleMute, reset, showFeedback, blip,
  }
